...
#if !( defined(ESP32) )
  #error This code is designed for (ESP32_S2/3, ESP32_C3 + ENC28J60) to run on ESP32 platform! Please check your Tools->Board setting.
#endif
...
void setup()
{
  ...
  Serial.println(ESP32_SC_ENC_MANAGER_VERSION);
  ...
  ESP32_SC_ENC_Manager ESP32_SC_ENC_manager("ConfigOnDoubleReset_TZ");

#if !USE_DHCP_IP
  // Set (static IP, Gateway, Subnetmask, DNS1 and DNS2) or (IP, Gateway, Subnetmask)
  ESP32_SC_ENC_manager.setSTAStaticIPConfig(EthSTA_IPconfig);
#endif
  ...
  if (initialConfig)
  {
    ...
    //ESP32_SC_ENC_manager.setConfigPortalTimeout(600);
